...
description: Run the mandatory sprint boundary workflow for
---

<!-- agent-notes: { ctx: "mandatory sprint boundary: retro + sweep + gate + passes", deps: [AGENTS.md, docs/methodology/personas.md, docs/methodology/phases.md, docs/tech-debt.md, docs/performance-budget.md], state: active, last: "coordinator@2026-02-28" } -->
...
2. **Project board configured:** Check that `AGENTS.md` has a `project-number` and `project-owner` set (not commented out / not blank). If missing: "No project board is configured in AGENTS.md. Run `/kickoff` to set one up, or add the project-number and project-owner manually."
...
4. **Sprint planning:** Invoke Pat for prioritization of the next sprint's backlog. For each item, identify whether it requires the Architecture Gate (see AGENTS.md § Architecture Gate) and note it in the sprint plan.
