...
Every proxy decision is logged in `.agents/handoff.md` under `## Proxy Decisions (Review Required)` with:
...
- Proxy decision log entries in `.agents/handoff.md` (when acting as human proxy)
