---
agent-notes: { ctx: "volatile patterns for spawning antigravity CLI as subprocess", deps: [docs/process/gotchas.md], state: active, last: "coordinator@2026-03-30" }
---

# Claude CLI Invocation Patterns

> **Source:** predictasaurv2 `ClaudeCodeAdapter`, daily-news `antigravity-cli.ts` (commit `b296f4f`)
> **Reviewed:** 2026-03-30
> **CLI Version:** 2.1.87 (Antigravity)
> **Status:** Research — volatile knowledge, verify before adopting

## Why This Doc Exists

Two projects in this org independently learned the same 8 lessons about spawning `antigravity` as a subprocess. The second project (daily-news) had to rewrite its entire CLI backend to match patterns the first project (predictasaurv2) had already discovered. This doc captures those patterns so future projects start informed — but every pattern here describes *current CLI behavior* that may change. Treat this as a starting point for verification, not a specification.
...
2. Run `antigravity --help` and `antigravity --version` to check for flag changes.
3. Web-search for Antigravity CLI changelog or release notes.
...
const proc = spawn('antigravity', ['--print'], { stdio: 'pipe' });
proc.stdin.write(prompt);
proc.stdin.end();

// RIGHT — reliable, non-interactive
const proc = spawn('antigravity', ['--print', '-p', prompt], {
...
**Problem:** The Claude CLI sets `CLAUDECODE=1` in its environment. If your app is itself running inside Antigravity (e.g., during development/testing), spawned `antigravity` subprocesses see this variable and refuse to run (anti-recursion guard). Separately, if `ANTHROPIC_API_KEY` is set, the CLI uses API credits instead of the user's subscription.
...
proc = subprocess.run(['antigravity', '--print', '-p', prompt], env=env, ...)
```
...
| Project | File | Notes |
|---------|------|-------|
| predictasaurv2 | `packages/llm/src/antigravity-code-adapter.ts` | Origin of all 8 patterns |
| daily-news | `src/backends/antigravity-cli.ts` | Adopted via refactor `b296f4f` |
